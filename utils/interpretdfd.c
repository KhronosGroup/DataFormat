                                     ...
                                     uint32_t *wordBytes)
{
    ...
    for (sampleCounter = 0; sampleCounter < numSamples; ++sampleCounter) {
        ...
        if (!determinedNormalizedness) {
            ...
            /* Note that interpreting sampleUpper as a floating point value by accessing the */
            /* address isn't compatible with our helper macros, so we have to hard-wire this. */
    ...
    if ((result & i_PACKED_FORMAT_BIT)) {
        ...
        *wordBytes = KHR_DFDVAL(BDFDB, BYTESPLANE0);
        for (sampleCounter = 0; sampleCounter < numSamples; ++sampleCounter) {
